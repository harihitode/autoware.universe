...
//
// The EKFLocalizerOCL class is EKFLocalizer with OpenCL implementation.
//
...
class EKFLocalizerOCL : public rclcpp::Node
{
public:
  EKFLocalizerOCL(const std::string & node_name, const rclcpp::NodeOptions & options);
...
private:
  ...
  bool use_double_;  //!< @brief  flag to use double in OpenCL kernel code or not
  ...
  friend class EKFLocalizerOCLTestSuite;  // for test code
};
#endif

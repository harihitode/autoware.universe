...
//
// The NormalDistributionsTransformOCL functions are based
...
: ndt_ptr_(new pclocl::NormalDistributionsTransform<PointSource, PointTarget>)
{
}
...
{
  ndt_ptr_->computeDerivativesCLCall_ = 0;
  ndt_ptr_->align(output, guess);
  std::cout << "CL call: " << ndt_ptr_->computeDerivativesCLCall_ << std::endl;
}
...
template <class PointSource, class PointTarget>
void NormalDistributionsTransformOCL<PointSource, PointTarget>::setTransformationEpsilon(
  double trans_eps)
{
...
template <class PointSource, class PointTarget>
double NormalDistributionsTransformOCL<
...
template <class PointSource, class PointTarget>
Eigen::Matrix4f NormalDistributionsTransformOCL<PointSource, PointTarget>::getFinalTransformation()
  const
{
...
template <class PointSource, class PointTarget>
Eigen::Matrix<double, 6, 6> NormalDistributionsTransformOCL<PointSource, PointTarget>::getHessian()
  const
{
...
template <class PointSource, class PointTarget>
double
NormalDistributionsTransformOCL<PointSource, PointTarget>::calculateTransformationProbability(const pcl::PointCloud<PointSource> & trans_cloud) const
{
  return ndt_ptr_->calculateTransformationProbability(trans_cloud);
}

template <class PointSource, class PointTarget>
double NormalDistributionsTransformOCL<PointSource, PointTarget>::
calculateNearestVoxelTransformationLikelihood(const pcl::PointCloud<PointSource> & trans_cloud) const
{
  return ndt_ptr_->calculateNearestVoxelTransformationLikelihood(trans_cloud);
}

template <class PointSource, class PointTarget>
void NormalDistributionsTransformOCL<PointSource, PointTarget>::setNumThreads(int n)
{
  ndt_ptr_->setNumThreads(n);
}

template <class PointSource, class PointTarget>
void NormalDistributionsTransformOCL<PointSource, PointTarget>::setNeighborhoodSearchMethod(
  pclocl::NeighborSearchMethod method)
{
  ndt_ptr_->setNeighborhoodSearchMethod(method);
}

template <class PointSource, class PointTarget>
int NormalDistributionsTransformOCL<PointSource, PointTarget>::getNumThreads() const
{
  return ndt_ptr_->getNumThreads();
}

template <class PointSource, class PointTarget>
pclocl::NeighborSearchMethod
NormalDistributionsTransformOCL<PointSource, PointTarget>::getNeighborhoodSearchMethod() const
{
  return ndt_ptr_->getNeighborhoodSearchMethod();
}

#endif  // NORMAL_DISTRIBUTIONS_TRANSFORM_OCL_HPP
